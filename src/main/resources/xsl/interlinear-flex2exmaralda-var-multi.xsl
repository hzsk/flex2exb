...
<!--        <xsl:variable name="textname" select="./interlinear-text/item[@type='title-abbreviation' and @lang='en']"></xsl:variable>-->
         <xsl:variable name="textname"
            select="current()/item[@type = $settings//project[@id=$language]/text-name/@from-item and @lang = $settings//project[@id=$language]/text-name/@from-lang]/text()"/> 
        <xsl:variable name="speaker"
            select="
                if (substring-before($textname, '_') = '') then
                    'SPK'
                else
                if ($settings//project[@id=$language]/speaker-code/@position = '2') then
                        substring-before(substring-after($textname, '_'), '_')
                    else
                        substring-before($textname, '_')"/>

        <xsl:variable name="comment"
            select="//item[@type='source' and @lang='en']/text()"/>
        ...
        <!--              <interlinear-text name="{$textname}"/>
            ...
            <basic-transcription>
                <head>
                    <meta-information>
                        <project-name>
                            <xsl:value-of select="$settings//project[@id=$language]/project-name/@value"/>
                        </project-name>
                        <transcription-name>
                ...
                <basic-body>
                    ...
                    <xsl:for-each select="$settings//project[@id=$language]/tiers/tier">
